...
function ErrorAlert({ errorCode }: { errorCode: string }) {
  if (errorCode === 'BANNED') {
    return (
      <div
        style={{
  ...
  const message =
    errorCode === 'UNAUTHORIZED'
      ? 'Your account is not authorized. Only @pascack.org Google accounts may sign in.'
...
function LoginCard() {
  ...
  const makeVariants = (index: number) => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          FRC Team 1676 · The Pi-oneers
        </span>
      </motion.div>

      {/* Error alert (above HR) */}
      {errorCode && (
      ...
      >
        ...
        >
          Secured with Google Workspace — @pascack.org only
        </span>
      </motion.div>
    </div>
  )
