...
app.prepare().then(() => {
    ...
    try {
      ...
      const req = {
        ...
        cookies: Object.fromEntries(
          cookieHeader.split(';').map((cookie) => cookie.trim().split('=').map(decodeURIComponent))
        ),
  ...
  io.on('connection', async (socket: Socket) => {
    ...
    socket.on('message:send', async (data: { channelId: string; content: string; threadId?: string }) => {
      const { channelId, content, threadId } = data
      const message = await prisma.message.create({
        data: { content, authorId: userId, channelId, threadId },
        include: {
