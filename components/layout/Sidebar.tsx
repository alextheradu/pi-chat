...
export function Sidebar({ channels, dms, currentUser }: SidebarProps) {
  ...
  return (
    <nav role="navigation" aria-label="Sidebar" style={{
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '8px 8px 0' }}>
        <div style={{ marginBottom: 8 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '2px 8px', marginBottom: 2 }}>
            <span style={{ fontSize: 10, fontFamily: 'var(--font-mono)', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.8px' }}>Channels</span>
            {canCreateChannel && (
              <button aria-label="Create channel" style={{ width: 20, height: 20, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'transparent', border: 'none', cursor: 'pointer', borderRadius: 4, color: 'var(--text-muted)' }}><Plus size={14} /></button>
            )}
          </div>

          {ungrouped.map(ch => (
            <SidebarChannelItem key={ch.id} id={ch.id} name={ch.name} isPrivate={ch.isPrivate} isAnnouncement={ch.isAnnouncement} unreadCount={unreadCounts[ch.id] ?? 0} />
          ))}
          ...
          {Object.values(grouped).map(({ sub, channels: subChannels }) => (
            <div key={sub.id}>
              <button
                ...
                style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 6, padding: '4px 8px', background: 'transparent', border: 'none', cursor: 'pointer', marginTop: 6 }}
              >
                <span style={{ width: 6, height: 6, borderRadius: '50%', background: sub.color, flexShrink: 0 }} />
                <span style={{ fontSize: 10, fontFamily: 'var(--font-mono)', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.8px', flex: 1, textAlign: 'left' }}>{sub.displayName}</span>
                {collapsedSubs.has(sub.id) ? <ChevronRight size={12} color="var(--text-muted)" /> : <ChevronDown size={12} color="var(--text-muted)" />}
              </button>
        ...
        <div style={{ marginBottom: 8 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '2px 8px', marginBottom: 2 }}>
            <span style={{ fontSize: 10, fontFamily: 'var(--font-mono)', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.8px' }}>Direct Messages</span>
            <button aria-label="New direct message" style={{ width: 20, height: 20, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'transparent', border: 'none', cursor: 'pointer', borderRadius: 4, color: 'var(--text-muted)' }}><Plus size={14} /></button>
          </div>
          {dms.map(dm => (
            <SidebarDMItem key={dm.id} id={dm.id} userId={dm.userId} name={dm.name} avatarUrl={dm.avatarUrl} status={dm.status} unreadCount={dm.unreadCount} />
          ))}
        </div>
      </div>

      <SidebarFooter userId={currentUser.id} name={currentUser.name} avatarUrl={currentUser.avatarUrl} status={currentUser.status} role={currentUser.role} />
    </nav>
  )
}
